...
ARCMaxGridSize = 30
ARCAugmentRetriesFactor = 5  # 生成足够独特增广的重试系数
...
def arc_grid_to_np(grid: List[List[int]]):
    # ARC 原始 grid -> uint8 numpy，并做边界检查
    arr = np.array(grid)
...
def np_grid_to_seq_translational_augment(inp: np.ndarray, out: np.ndarray, do_translation: bool):
    ...
    # 将二维 grid 平铺为固定 30x30 序列，并可做平移增广
...
def convert_dataset(config: DataProcessConfig):
    ...
    # 映射全局 puzzle identifier（用于 puzzle embedding）
