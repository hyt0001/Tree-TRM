...
def convert_subset(set_name: str, config: DataProcessConfig):
    ...
    csv_path = None
    if config.local_dir and os.path.exists(os.path.join(config.local_dir, f"{set_name}.csv")):
        csv_path = os.path.join(config.local_dir, f"{set_name}.csv")
        ...
        try:
            ...
            if os.path.exists(f"{set_name}.csv"):
                csv_path = f"{set_name}.csv"
            else:
                raise
    ...
    # 转为 numpy 并映射到 token 区间（+1 为了保留 PAD=0）
