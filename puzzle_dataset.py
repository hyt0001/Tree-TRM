...
def _sample_batch(rng: np.random.Generator, group_order: np.ndarray, puzzle_indices: np.ndarray, group_indices: np.ndarray, start_index: int, global_batch_size: int):
    # 依据“组→题目→样本”的层级采样，尽量凑满一个全局 batch
    batch = []
    batch_puzzle_indices = []
    current_size = 0

    while (start_index < group_order.size) and (current_size < global_batch_size):
        # 从当前 group 随机选一个 puzzle，再从该 puzzle 中抽样样本
        group_id = group_order[start_index]
        puzzle_id = rng.integers(group_indices[group_id], group_indices[group_id + 1])
        start_index += 1

        # 当前 puzzle 在扁平样本数组中的范围
        puzzle_start = puzzle_indices[puzzle_id]
        puzzle_size = int(puzzle_indices[puzzle_id + 1] - puzzle_start)

        append_size = min(puzzle_size, global_batch_size - current_size)

        # 把该 puzzle 的样本索引与 puzzle_id 一起写入 batch
...
class PuzzleDataset(IterableDataset):
    def __init__(self, config: PuzzleDatasetConfig, split: str = "train"):
        ...
        # 分布式：global_batch_size 必须可被节点数整除
    ...
    def _lazy_load_dataset(self):
        ...
        # 延迟加载：inputs/labels 走 memmap，索引常驻内存
        self._data = {}
    ...
    def _collate_batch(self, batch):
        ...
        # 将数据集中定义的 ignore_label_id 统一映射为训练用 IGNORE_LABEL_ID
        if self.metadata.ignore_label_id is not None:
            batch["labels"][batch["labels"] == self.metadata.ignore_label_id] = IGNORE_LABEL_ID

        # 如果末尾 batch 不足 local_batch_size，进行 padding（inputs/labels/identifiers）
    ...
    def _iter_test(self):
        for set_i, (set_name, dataset) in enumerate(self._data.items()):  # type: ignore
            ...
            while start_index < total_examples:
                ...
                # 同步计算每个样本对应的 puzzle_id（用于 puzzle embedding）
                puzzle_indices = []
    ...
    def _iter_train(self):
        for set_name, dataset in self._data.items():  # type: ignore
            # 训练模式：按 group 打乱，提升多样性
            self._iters += 1

            # 以 group 为单位的随机顺序，可重复 epochs_per_iter 次以减少 DataLoader 开销
